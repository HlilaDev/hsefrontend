...
export interface Device {
  _id?: string;              // ✅ optionnel côté front
  deviceId: string;
  name?: string;
  zone: ZoneRef;
...
export type CreateDevicePayload = {
  deviceId: string;
  name?: string;
  zone: string;              // ✅ on envoie l'id de la zone
  sensors: string[];
  status: DeviceStatus;
  description?: string;
};
...
export class DeviceServices {
  ...
  // Helpers pour éviter /devices//:id
  ...
  editDevice(id: string, payload: UpdateDevicePayload): Observable<any> {
    return this.http.put<any>(this.byId(API_URLS.devices.editDevice, id), payload);
  }
